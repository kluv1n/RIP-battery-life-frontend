...
export async function authLoginRequest(credentials: SignInRequest) {
  return authAxios.post<{ token?: string }>("/users/signin", credentials);
}

export async function authRegisterRequest(user: SignUpRequest) {
  return authAxios.post("/users/signup", user);
}

export async function authLogoutRequest() {
  return authAxios.post("/users/signout", null, { validateStatus: (s) => s === 204 || s < 300 });
}
